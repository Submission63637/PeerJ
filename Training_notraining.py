...
def load_group(file_obj):
    ...
    if (text_sz == 0):
        return -1, -1, -1
    dt = np.dtype('<i4')
    sentence = np.frombuffer(file_obj.read(4 * text_sz), dtype=dt).copy().tolist()
...
class BertPeacock(nn.Module):
    ...
    def __init__(self, bertmod_pretrained, group_embedding, use_group_ids, peacock_dim=32, peacock_i=6):
        super().__init__()
...
def main():
    ...
    opt = AdamW(tot_model.parameters(), lr=lr)
